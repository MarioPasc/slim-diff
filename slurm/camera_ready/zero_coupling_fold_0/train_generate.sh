#!/usr/bin/env bash
#SBATCH -J slimdiff_cr_zero_coupling_fold_0
...
# =============================================================================
# SLIM-Diff SASHIMI 2026: zero-coupling baseline (IndependentTwinDDPM) × fold 0
# =============================================================================
# Zero-coupling point on the coupling continuum. Two independent single-channel
# DDPMs sharing only the conditioning embedding. See:
#   docs/sashimi26/01_zero_coupling_baseline.md
#
# Reproducibility:
#   - SEED_BASE=42 is shared across ALL cells so that x_T noise realisations
...
#     same fold (the SHA256 seeding is architecture-agnostic).
#   - Fold CSVs are produced by `slimdiff-kfold` with seed=42; the first
#     job to run builds them atomically, subsequent jobs skip.
# =============================================================================
...
# --- Picasso paths (override via env; launch_zero_coupling.sh exports -------
...
RESULTS_ROOT="${RESULTS_ROOT:-/mnt/home/users/tic_163_uma/mpascual/fscratch/results/sashimi2026}"
...
# --- Generation phase parameters (held constant across all cells) -----------
NUM_REPLICAS=20
...
if [ "$GPU_COUNT" -lt 2 ]; then
    echo "WARNING: config requests 2 GPUs but only ${GPU_COUNT} available"
