...
CACHE_CONFIG_FILE="${REPO_SRC}/src/diffusion/config/cache/${DATASET}.yaml"
...
# 3. Handle slice cache creation (if needed)
if [ ! -d "${CACHE_DIR}" ]; then
    echo "Cache directory not found. Preparing cache configuration..."
    
    # Copy and modify the cache configuration file
    CACHE_CONFIG_BASENAME=$(basename "${CACHE_CONFIG_FILE}")
    MODIFIED_CACHE_CONFIG="${RESULTS_DST}/${CACHE_CONFIG_BASENAME}"
    
    echo "Copying cache config file to: ${MODIFIED_CACHE_CONFIG}"
    cp "${CACHE_CONFIG_FILE}" "${MODIFIED_CACHE_CONFIG}"
    
    echo "Modifying cache configuration file..."
    sed -i "s|cache_dir: .*|cache_dir: \"${CACHE_DIR}\"|" "${MODIFIED_CACHE_CONFIG}"
    sed -i "s|root_dir: .*|root_dir: \"${DATA_SRC}/source\"|" "${MODIFIED_CACHE_CONFIG}"
    
    echo "Running caching step with config: ${MODIFIED_CACHE_CONFIG}"
    cd "${REPO_SRC}"
    jsddpm-cache --config "${MODIFIED_CACHE_CONFIG}"
...
# 4. Backup copy the training configuration file to the execution folder
...
# 5. Modify the training configuration file
echo "Modifying training configuration file..."
sed -i "s|  root_dir: .*|  root_dir: \"${DATA_SRC}\"|" "${MODIFIED_CONFIG}"
sed -i "s|  cache_dir: .*|  cache_dir: \"${CACHE_DIR}\"|" "${MODIFIED_CONFIG}"
